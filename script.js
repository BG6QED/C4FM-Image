...
function colorToRGB(str){ let r=hexToRgb(str); if(r) return r;
...
function dec2Hex(v){ const n=v%100; return (n%10)+16*Math.floor(n/10); }
...
function getDateTaken(exif){
    try{if(exif&&exif[36867]){const s=exif[36867].description||exif[36867];return new Date(s.replace(/:/g,'-'));}}
...
function encodeGPS(exif){
    const g=getGeotagging(exif); if(!g) return ' ';
    try{
...
function resetTextOptions(){
    document.getElementById('text').value=''; document.getElementById('color').value='#ff0000';
    document.getElementById('fontSize').value=48;
    textOffsetX = 5; textOffsetY = 5;
    document.getElementById('textX').value=5; document.getElementById('textY').value=5;
    document.getElementById('fontSizeValue').textContent='48px';
    document.getElementById('textXValue').textContent='5'; document.getElementById('textYValue').textContent='5';
    updatePreview();
}
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('resolution').addEventListener('change',()=>{
        const [w,h]=document.getElementById('resolution').value.split('x').map(Number);
        document.getElementById('textX').max=w; document.getElementById('textY').max=h;
    });
});
...
async function processImages(){
    ...
    const radioidStr = parseInt(document.getElementById('radioid').value).toString().padStart(8,'0');
    ...
    const size = parseInt(document.getElementById('fontSize').value);
    const tx = textOffsetX, ty = textOffsetY;   // 直接使用拖拽得到的坐标
    ...
    gallery.innerHTML=''; progBar.style.width='0%';
    ...
    }catch(e){ statusEl.textContent='批量处理错误: '+e.message; progDiv.style.display='none'; }
}